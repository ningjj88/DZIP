    ...
    private void getLastFile() {
        try {
            ...
            if (file.isDirectory()) { //是个目录
                files.add(new File(fileUrl + File.separator + "atm_" + trace + ".log"));
            } else {
                files.add(new File(fileUrl + "_" + trace + ".log"));
            }
        ...
        try {
            ...
            while ((line = br.readLine()) != null) {
                if (line.trim().length() != 0) {
                    if (file.isDirectory()) { //是个目录
                        fileList.add(new File(fileUrl + File.separator + "atm" + "_" + line + ".log"));
                    } else {
