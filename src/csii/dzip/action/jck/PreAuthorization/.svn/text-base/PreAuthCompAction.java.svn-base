...
/**
 *@ClassName:&{type_name}
 *@Description：POS预授权完成
 *@author :chenshq
 *@date：2010-08-16
 */
public class PreAuthCompAction extends DzipBaseAction {
	private UpdateJoural updateJoural;
	private ActionUtilProcessor utilProcessor;

	@SuppressWarnings("unchecked")
	public void execute(final Context ctx) throws PeException {
		logger.info("JCK PreAuthCompleteAction ==================>Start");
		...
		logger.info("JCK预授权完成上笔交易信息(sqlMap)==================>"+sqlMap);
		...
		if(sqlMap!=null){
			...
			BigDecimal resultAmt=preAmt.multiply(InitData4Dzip.getPreAuthCompProp());  //只计算比较
			//if(tranAmt <=resultAmt){  //预授权完成的金额小于预授权金额的1倍
			if(tranAmt.compareTo(resultAmt) < 0){  //预授权完成的金额小于预授权金额的1倍
...
//							procedureMap.put(Constants.PE_POST_DATE, ctx.getData(Constants.PE_POST_DATE));  //原 账务日期 
				...
				logger.info("==================>JCK预授权完成的金额大于预授权金额的1.15倍时，交易失败");
				ctx.setData(Constants.PE_TRANS_STAT, Constants.PE_EIGHT);  //交易状态：成功
			...
			logger.info("==================>JCK预授权完成上笔交易已经过期(超过30天在前置流水中找不到该记录)");
			ctx.setData(Constants.PE_TRANS_STAT, Constants.PE_EIGHT);  //交易状态：成功
			ctx.setData(Constants.FIX8583_RESP,Constants.PE_22);    //响应码
			ctx.setData(Constants.PE_HOST_RESP_CD,Constants.PE_22);    //响应码
			ctx.setData(Constants.FIX8583_EXPI, Constants.PE_EXPIREYEAR);
