 ...
 * @desc 更改渠道交易关系
 */
public class UpdChannelTranAction extends DzipBaseAction{
	private JdbcAccessAwareProcessor dzipdbAccess;

	@SuppressWarnings("unchecked")
	@Override
	public void execute(final Context ctx) throws PeException {
		logger.info("UpdChannelTranAction==========================>Start");
		final Map paramMap = new HashMap();
		List listRe = new ArrayList();
		dzipdbAccess.getTransactionTemplate().execute(new TransactionCallback() {
			public Object doInTransaction(TransactionStatus arg0) {
				paramMap.put(Constants.IN_TRANCD, ctx.getData(Constants.TRANCD));
				paramMap.put(Constants.IN_CHANNID, ctx.getData(Constants.CHANNID));
				paramMap.put(Constants.IN_TRANDESC, ctx.getData(Constants.TRANDESC));
				paramMap.put(Constants.IN_TRANSTAT, ctx.getData(Constants.TRANSTAT));
				dzipdbAccess.getSqlMap().update("platformAdmin.UpdChannelTran", paramMap);
				return null;
			}
		});
		if (Constants.PE_ZERO.equals(String.valueOf(paramMap.get(Constants.OUT_ERROR_NBR)))) {
			...
			if (String.valueOf(paramMap.get(Constants.OUT_ERROR_NBR)).equals(Constants.Error_99999)) {
				...
				throw new PeException(Errors.UPDATE_CHANNEL__TRAN_FAILED);
			} else {
