 ...
 * @desc 通存通兑流水表
 */
public class InqDepAndWthJournalAction extends DzipBaseAction{
	private JdbcAccessAwareProcessor dzipdbAccess;
	@SuppressWarnings("unchecked")
	@Override
	public void execute(final Context ctx) throws PeException {
		logger.info("InqDepAndWthJournalAction==========================>Start");
		List listRe = new ArrayList();
	    final Map paramMap = new HashMap();
	    dzipdbAccess.getTransactionTemplate().execute(new TransactionCallback() {
			public Object doInTransaction(TransactionStatus arg0) {
				Init.initDepAndWthJournal(ctx, paramMap);
				dzipdbAccess.getSqlMap().queryForObject("platformAdmin.GetDepAndWthJournal", paramMap);
				return null;
			}
		});
