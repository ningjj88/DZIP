 ...
 *@date：2012-12-27
 */
public class NoCardDepositAction extends DzipBaseAction {
	...
	public void execute(final Context ctx) throws PeException {
		logger.info("NoCardDepositAction=======================>Start");
		ctx.setData(Constants.IN_BUSITYP, Constants.PE_01); 	//业务类型:01_现金及转账
		utilProcessor.valTranLimit(ctx);//验证交易限制
		ctx.setData(Constants.CHECK_TRACK2, Constants.PE_N); 	//验证二磁
		ctx.setData(Constants.CHECK_PIN, Constants.PE_N); 	//验证密码
