...
public class Debit4LoadAction extends DzipBaseAction {
	...
	public void execute(Context ctx) throws PeException {
		if(dzipProcessTemplate.queryParam(Constants.RCVG_CD_YLSJ).equals(ctx.getData(Constants.ISO8583_SOURID))){//如果是银联数据发来的报文
			logger.info("Debit4LoadAction(本代本圈提/柜面转待补登)=======================>Start");
			...
			if(Constants.PE_OK.equals(ctx.getData(Constants.PE_HOST_RESP_CD))){//如果应答成功
				...
				if(Constants.PE_OK.equals(responcd)){
					...
					if(currentAmt.compareTo(currentBal) > Constants.PE_NUM_ZERO){//交易金额大于实际电子钱包余额
						...
						throw new PeException("圈提金额大于电子钱包余额, " + "圈提金额 = " + currentAmt + ", 电子钱包余额 = " + currentBal);
					}
					Map<String, Object> map = utilProcessor.getJournalInfo(String.valueOf(ctx.getData(Constants.PE_JOURNAL_NO)));
					if(null != map.get(Constants.PE_ACCTID1)
							&& !Constants.PE_NULL.equals(map.get(Constants.PE_ACCTID1))){//判断交易是否是来自柜面的补登圈存
						logger.info("=================>柜面转待补登!");
