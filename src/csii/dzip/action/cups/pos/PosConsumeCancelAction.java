...
public class PosConsumeCancelAction extends DzipBaseAction {
	...
	public void execute(final Context ctx) throws PeException {
		logger.info("CUPS PosConsumeCancelAction ==================>Start");
		ctx.setData(Constants.IN_BUSITYP, Constants.PE_03); 	//业务类型:03_消费及预授权
		Map procedureMap = new HashMap();
		//业务规则中无有卡自助消费撤消交易，该交易发卡方应该返40
		if(Constants.PE_Y.equals(ctx.getData(Constants.CHECK_TRACK2))
		   && Constants.PE_TWO.equals(ctx.getString(Constants.IN_TRANLAUNCHWAYCD))){
			ctx.setData(Constants.PE_HOST_RESP_CD, Constants.PE_CHANN_INVALIDATE);          //响应码
			ctx.setData(Constants.ISO8583_RESCODE, Constants.PE_CHANN_INVALIDATE);          //响应码
			throw new PeException("业务规则中无有卡自助消费撤消交易");
		}
		...
		if(ctx.getData(Constants.ISO8583_ORGDATA)!=null){
			...
			if(origTranDate.equals(ctx.getData(Constants.ISO8583_LOCTRANSDATE))){      // 防止隔日撤销
				...
				if(count>0){
					...
					logger.info("CUPS消费撤销上笔交易信息(resultMap)==================>"+resultMap);
