...
/**
 *@ClassName:&{type_name}
 *@Description:他行存款撤销冲正
 *@author :xujin
 *@date：2010-08-16
 */
public class DepositATCDMCancelReversalAction extends DzipBaseAction {
	private UpdateJoural updateJoural;
	private ActionUtilProcessor  utilProcessor;
	
	@SuppressWarnings("unchecked")
	public void execute(final Context ctx) throws PeException {
		logger.info("CUPS DepositATCDMCancelReversalAction ==================>Start");
		...
		if(count>0){
			...
			logger.info("CUPS冲正撤销冲正上笔交易信息(resultMap)==================>"+resultMap);
			...
			if(Constants.PE_ONE.equals((String)ctx.getData(Constants.PE_OUTSIDEFLAG))){//境外交易
				if(ctx.getData(Constants.ISO8583_CHDAMT)==null)//F6为null,交易金额为存款撤销交易金额
