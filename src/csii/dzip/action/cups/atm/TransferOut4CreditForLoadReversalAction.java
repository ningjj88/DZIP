 ...
 * 他带本非指定账户转出圈存冲正
 * @author xiehai
 * @date 2013-11-5 上午10:07:02
 */
public class TransferOut4CreditForLoadReversalAction extends DzipBaseAction {
	...
	public void execute(Context ctx) throws PeException {
		logger.info("TransferOut4CreaditForLoadReversalAction(他带本非指定账户转出圈存冲正交易)====================>Start!");
		...
		if(count > 0){//如果冲正交易已经发生
			...
			if(Constants.PE_ONE.equals((String)ctx.getData(Constants.PE_OUTSIDEFLAG))){//境外交易
				if(null == ctx.getData(Constants.ISO8583_CHDAMT)){//F6为null,交易金额为转出交易金额
