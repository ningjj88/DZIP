...
public class CashRecharge4CreditForLoadCancelReversalAction extends DzipBaseAction {
	...
	public void execute(Context ctx) throws PeException {
		logger.debug("CashRecharge4CreditForLoadCancelReversalAction(他带本现金充值圈存撤销冲正)====================>Start!");
		...
		if(count > 0){
			...
			logger.info("他带本现金充值圈存撤销冲正上笔交易信息(resultMap)==================>"+resultMap);
			...
			if(Constants.PE_OK.equals(ctx.getData(Constants.ISO8583_RESCODE))){
				procedureMap.put(Constants.PE_TRANS_STAT, Constants.PE_ZERO);//交易状态：成功 现金充值圈存
				procedureMap.put(Constants.PE_JOURNAL_NO, resultMap.get(Constants.PE_RLTSEQNO));//现金充值圈存流水号
				updateJoural.execute(procedureMap);
				ctx.setData(Constants.PE_RLTSEQNO, resultMap.get(Constants.JOURNAL_NO));//原交易流水号
				procedureMap.clear();
				procedureMap.put(Constants.PE_TRANS_STAT, Constants.PE_NINE );   //交易状态: 已冲正 现金充值圈存撤销
