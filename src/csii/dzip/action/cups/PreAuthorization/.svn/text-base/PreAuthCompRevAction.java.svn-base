...
public class PreAuthCompRevAction extends DzipBaseAction {
	...
	public void execute(final Context ctx) throws PeException {
		logger.info("CUPS PreAuthCompRevAction ==================>Start");
		...
		if(count>0){
			...
			logger.info("CUPS预授权完成冲正上笔交易信息(resultMap)==================>"+resultMap);
			if(Constants.PE_ONE.equals((String)ctx.getData(Constants.PE_OUTSIDEFLAG))){//境外交易
				if(ctx.getData(Constants.ISO8583_CHDAMT)==null)//F6为null,交易金额为预授权完成交易金额
			...
			logger.info("CUPS预授权完成冲正上笔的上笔交易信息(sqlMap)==================>"+sqlMap);
...
//					procedureMap.put(Constants.PE_POST_DATE, ctx.getData(Constants.PE_POST_DATE));  //原 账务日期 
					ctx.setData(Constants.PE_RLTSEQNO,resultMap.get(Constants.JOURNAL_NO));           	//原交易流水号
					ctx.setData(Constants.ISO8583_RESCODE,Constants.PE_OK);         //原交易流水号
