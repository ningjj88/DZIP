...
public class PreAuthCompAction extends DzipBaseAction {
	...
	public void execute(final Context ctx) throws PeException {
		logger.info("CUPS PreAuthCompleteAction ==================>Start");
		...
		logger.info("CUPS预授权完成上笔交易信息(sqlMap)==================>"+sqlMap);
		if(sqlMap!=null){
			...
			if(Constants.PE_ZERO.equals(tranStat)){              //0：表示预授权成功
				...
				if(Constants.PE_OK.equals(ctx.getData(Constants.ISO8583_RESCODE))){  //客户帐扣除成功
...
//							procedureMap.put(Constants.PE_POST_DATE, ctx.getData(Constants.PE_POST_DATE));  //原 账务日期 
			...
			logger.info("==================>CUPS预授权完成上笔交易已经过期(超过30天在前置流水中找不到该记录)");
