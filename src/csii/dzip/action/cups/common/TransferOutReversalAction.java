...
/**
 *@ClassName:&{type_name}
 *@Description: 他行转出记账冲正：ATM，CDM
 *@author :xujin
 *@date：2010-09-07
 */
public class TransferOutReversalAction extends DzipBaseAction {
	private UpdateJoural updateJoural;
	private ActionUtilProcessor  utilProcessor;

	@SuppressWarnings("unchecked")
	public void execute(final Context ctx) throws PeException {
		logger.info("CUPS TransferOutReversalAction ==================>Start");
		...
		if(count>0){
			...
			if(Constants.PE_ONE.equals((String)ctx.getData(Constants.PE_OUTSIDEFLAG))){//境外交易
				if(ctx.getData(Constants.ISO8583_CHDAMT)==null)//F6为null,交易金额为转出交易金额
