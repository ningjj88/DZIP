 ...
 *@date：20112-03-017
 */
public class CollectionCancelReversalAction extends DzipBaseAction {
	private UpdateJoural updateJoural;
	private ActionUtilProcessor  utilProcessor;
	
	@SuppressWarnings("unchecked")
	public void execute(final Context ctx) throws PeException {
		logger.info("CUPS CollectionCancelReversalAction ==================>Start");
		...
		if(count>0){
			...
			logger.info("CUPS代收撤销冲正上笔交易信息(resultMap)==================>"+resultMap);
			if(Constants.PE_ONE.equals((String)ctx.getData(Constants.PE_OUTSIDEFLAG))){//境外交易
				if(ctx.getData(Constants.ISO8583_CHDAMT)==null)//F6为null,交易金额为代收撤销交易金额
